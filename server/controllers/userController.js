  ...
  try {
    ...
    if (data.rows[0]) {
      return next({
        ...
        message: {
          err: 'An error occured on userController.findUserEmail: User already exists',
        },
      });
    } else {
      return next();
...
userController.authenticateUser = async (req, res, next) => {
  try {
    ...
    let authenticateUserQuery =
      'SELECT * FROM public.user WHERE public.user.email = $1';
    const data = await db.query(authenticateUserQuery, [email]); //need to test login with incorrect email, better error handling
    ...
    if (data.rows[0] && verifyPassword) {
      ...
      return next({
        ...
        message: {
          err: 'An error occured on userController.authenticateUser: User email or password does not match',
        },
      });
    }
  } catch (err) {
